...
1. **login mutation (lines 166-318):**
   ...
   - ADD: Session persistence pattern with proper error handling:
     ```typescript
     // Set session
     ...
     // Save session with error handling (reference: auth.ts lines 99, 205)
     try {
...
2. **verifyMagicLink mutation (lines 431-556):**
   ...
   - Call req.session.save() with try-catch wrapper:
     ```typescript
     // Set session
     (ctx.req.session as any).clientPortalClientId = account.clientId;
     (ctx.req.session as any).clientPortalOrganizationId = organizationId;
...
4. **logout mutation (lines 736-777):**
   ...
   - ADD: Session destruction with error handling:
     ```typescript
...
**WHY error handling matters:** If Redis connection fails during session save, the mutation would throw an unhandled exception causing generic 500 error. With try-catch wrapper, we get console.error logging for debugging production session issues and user-friendly error messages. This prevents partial state where session data is set in memory but save failed.
...
- login mutation calls req.session.save() with try-catch wrapper and error logging
- verifyMagicLink mutation uses same session pattern with error handling
- logout mutation destroys session with try-catch wrapper
- All session.save() calls have console.error logging for debugging
