...
must_haves:
  truths:
    ...
    - "Production deployment scripts tested and work with current schema"
  artifacts:
...
<objective>
Archive obsolete database scripts, test production deployment scripts, and update documentation to guide developers toward current best practices.

Purpose: Clean up scripts directory so developers don't accidentally use outdated patterns, prevent future migration debugging sessions, and ensure deploy-*.sh scripts work with current schema.

Output: Archived directory with obsolete scripts + documentation, tested deployment scripts, updated README.md with clear usage guidance.
</objective>
...
<task type="auto">
  <name>Test production deployment scripts</name>
  <files>
    packages/database/scripts/deploy-master.sh
    packages/database/scripts/deploy-tenants.sh
  </files>
  <action>
    Test production deployment scripts work with current schema (7 master + 31 tenant tables):

    **Test deploy-master.sh:**
    ```bash
    # Create test master database
    createdb rsm_master_deploy_test

    # Test deployment (dry run to see what would happen)
    DATABASE_URL="postgresql://postgres@localhost:5432/rsm_master_deploy_test" \
      bash packages/database/scripts/deploy-master.sh

    # Verify resulting table count
    psql -U postgres -d rsm_master_deploy_test -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';"
    # Should show 7 tables
    ```

    **Test deploy-tenants.sh:**
    ```bash
    # Create test tenant database
    createdb tenant_deploy_test

    # Test deployment
    bash packages/database/scripts/deploy-tenants.sh \
      "postgresql://postgres@localhost:5432" \
      "tenant_deploy_test"

    # Verify resulting table count
    psql -U postgres -d tenant_deploy_test -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';"
    # Should show 31 tables
    ```

    **If scripts work:**
    - Document success in audit-report.md (Status: WORKING ✅)

    **If scripts fail:**
    - Document specific errors
    - Fix issues (migration path, config references)
    - Retest until working

    **Cleanup:**
    ```bash
    dropdb rsm_master_deploy_test
    dropdb tenant_deploy_test
    ```
  </action>
  <verify>
    # Verify test databases cleaned up
    psql -U postgres -l | grep -E "(rsm_master_deploy_test|tenant_deploy_test)"
    # Should output nothing (databases dropped)

    # Verify audit report updated with deployment test results
    grep -E "deploy-(master|tenants).sh.*Status:" packages/database/scripts/audit-report.md
  </verify>
  <done>
    Production deployment scripts tested, work with current schema (7 master + 31 tenant tables), test databases cleaned up
  </done>
</task>

<task type="auto">
  <name>Archive obsolete scripts</name>
  <files>
    packages/database/scripts/archived/README.md
  </files>
  <action>
...
<task type="auto">
  ...
  <action>
    ...
    **Deployment scripts tested:** Phase 21 verified both deploy-master.sh and deploy-tenants.sh work with current schema (7 master + 31 tenant tables).
  ...
  <verify>
    ...
    # Verify deployment testing mentioned
    grep "Deployment scripts tested" packages/database/scripts/README.md
  </verify>
  <done>
    README.md rewritten with Phase 21 guidance, clear quick start, references to DEVELOPMENT-WORKFLOW.md, deployment testing documented, archived scripts documented
  </done>
</task>
...
Documentation:
- README.md: Updated with Phase 21 guidance, quick start, deployment testing
...
- Production deployments verified working
...
EOF
  ...
  <done>
    All Phase 21 script changes committed, comprehensive commit message documents audit results, deployment testing, and new patterns
  </done>
</task>
...
grep "Deployment scripts tested" scripts/README.md
...
<success_criteria>
- ✅ Production deployment scripts tested (deploy-master.sh, deploy-tenants.sh)
- ✅ Both deployment scripts work with current schema (7 master + 31 tenant tables)
- ✅ 7 obsolete scripts moved to archived/ directory
- ✅ archived/README.md explains why each script obsolete
- ✅ Main README.md updated with Phase 21 guidance and deployment testing results
