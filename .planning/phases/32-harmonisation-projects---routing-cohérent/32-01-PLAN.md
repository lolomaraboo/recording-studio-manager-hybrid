...
must_haves:
  truths:
    ...
    - "Inline edit form in ProjectDetail replaced with accordion component"
  ...
  artifacts:
    ...
    - path: "packages/client/src/pages/ProjectDetail.tsx"
      provides: "Detail page with ProjectEditForm in edit mode"
      contains: "ProjectEditForm"
  key_links:
    ...
    - from: "ProjectDetail.tsx"
      to: "ProjectEditForm"
      via: "reusable component for editing"
      pattern: "ProjectEditForm"
...
@packages/client/src/pages/ProjectDetail.tsx (inline edit form lines 353-473 - to be replaced)
...
<task type="auto">
  <name>Task 3: Refactor ProjectDetail.tsx edit mode to use ProjectEditForm</name>
  <files>packages/client/src/pages/ProjectDetail.tsx</files>
  <action>
**Pattern:** Replace inline edit form (lines 353-473) with ProjectEditForm component in edit mode.

**Current state:** ProjectDetail.tsx (1124 lines) has an inline edit form with only 11 fields (name, artistName, description, genre, type, status, targetDeliveryDate, actualDeliveryDate, budget, totalCost, label, notes). Missing: clientId, catalogNumber, startDate, endDate, trackCount, coverArtUrl, storageLocation, storageSize, spotifyUrl, appleMusicUrl, technicalNotes.
...
2. **Expand formData state** to include ALL 20+ fields (add missing fields with defaults)

3. **Update useEffect** that populates formData from project to include all fields

4. **Replace inline edit form** (the `{isEditing ? (<> ... </>)` block inside "Informations du projet" Card, lines 353-473) with:
   ```typescript
   {isEditing ? (
     <ProjectEditForm
       formData={formData}
     ...
     /* Keep existing read-only display as-is */
   )}
   ```

5. **Update handleSave** to pass all 20+ fields to updateMutation (currently only passes 11)

6. **Remove unused imports** from ProjectDetail: Input, Label, Textarea, Select components (now in ProjectEditForm) - but KEEP them if they're used elsewhere in the file (track form, status selector, etc.)

**Important:** The read-only display mode stays unchanged - only the edit form is replaced.

**Result:** Edit mode in ProjectDetail now exposes ALL 20+ fields via accordions (same UX as create), while read-only mode stays as-is.
  </action>
  <verify>
1. Navigate to /projects/:id
2. Click "Modifier" → ProjectEditForm renders with 5 accordions
3. All fields pre-filled from project data
4. Edit fields across all accordions
5. Click "Enregistrer" → updateMutation fires with all fields
6. Read-only mode unchanged
7. TypeScript 0 errors
  </verify>
  <done>
- ProjectDetail.tsx edit mode uses ProjectEditForm
- Inline edit form eliminated (120+ lines removed)
- All 20+ fields accessible in edit mode
- formData expanded to full field set
- handleSave passes all fields
- Read-only display mode unchanged
- TypeScript compiles successfully
  </done>
</task>

<task type="auto">
  <name>Task 4: Build and verify full harmonization</name>
  <files>packages/client/src/components/ProjectEditForm.tsx, packages/client/src/pages/ProjectCreate.tsx, packages/client/src/pages/ProjectDetail.tsx</files>
  <action>
...
3. **Test complete workflows:**
   - **Create:** Visit /projects/new → fill fields across 5 accordions → submit → success
   - **Edit:** Visit /projects/:id → click "Modifier" → ProjectEditForm with pre-filled data → edit → save → success
   - **Read-only:** After save, read-only display unchanged

4. **Document completion in commit:**
   ```bash
   git add packages/client/src/components/ProjectEditForm.tsx packages/client/src/pages/ProjectCreate.tsx packages/client/src/pages/ProjectDetail.tsx
   git commit -m "feat(32-01): replace inline forms with accordion-based ProjectEditForm
...
- Refactored ProjectDetail.tsx edit mode to use ProjectEditForm
- Inline form pattern eliminated from both create and edit flows
- All accordions open by default (Alt+Click toggle all)
- localStorage persistence for accordion state
- Client dropdown integrated with tRPC query
- Edit mode now exposes all 20+ fields (was only 11)
...
- ✓ Edit workflow functional (detail page)
- ✓ Inline forms eliminated from BOTH create and edit flows
- ✓ All 20+ project fields accessible in both modes
...
3. Create workflow functional (all 5 accordions)
4. Edit workflow functional (pre-filled data, save works)
5. Read-only display mode unchanged
6. Form state synchronization working
7. Client dropdown working
8. Date fields handling correctly
9. Git commit created with Phase 32-01 completion message
  </verify>
  <done>
- TypeScript 0 errors confirmed
- Production build successful
- ProjectEditForm harmonized with TalentEditForm patterns
- Inline forms eliminated from create AND edit flows
- Phase 32-01 complete
- Projects create+edit UI matches Talents/Clients harmonization
  </done>
</task>
...
3. **ProjectDetail page (edit mode):**
   - [x] Inline edit form replaced with ProjectEditForm
   - [x] formData expanded to all 20+ fields
   - [x] handleSave updated to pass all fields
   - [x] Read-only display mode unchanged
   - [x] Edit mode exposes all fields (was only 11)

4. **Build quality:**
   - [x] TypeScript 0 errors
   - [x] Client builds successfully
   - [x] Create and edit workflows functional
</verification>
...
- [ ] ProjectDetail.tsx edit mode refactored to use ProjectEditForm
- [ ] Inline form pattern eliminated from both create and edit flows
- [ ] Edit mode exposes all 20+ fields (was only 11 before)
- [ ] Client dropdown working with tRPC query
- [ ] Date fields handling correctly
- [ ] TypeScript 0 errors
- [ ] Client builds successfully
- [ ] Create and edit workflows functional
- [ ] Read-only display mode in ProjectDetail unchanged
- [ ] Pattern matches TalentEditForm Phase 28-05 (accordion-based complex forms)
- [ ] Phase 32-01 complete: Projects create+edit harmonized with Talents/Clients
</success_criteria>
